...
Součástí práce je prototypová implementace architektury navržené v kapitole \ref{sec:des_api}, konkrétně komponent \textit{Doménový model, Databázová vrstva, Perzistentní vrstva a Vrstva datového přístupu}. Na základě prototypové implementace je provedena validace navržené architektury a navrženého \textit{API} perzistentní vrstvy. V této kapitole jsou popsány důležité či nestandardní části implementace a je diskutována validnost vytvořeného návrhu.
