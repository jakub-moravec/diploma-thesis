...
Operace se chová různě v případě, kdy je umožňěno verzovaní metadatového úložiště (a to tak obsahuje více revízí) a kdy je vypnuto. V případě zapnutého verzování je \textit{merge} prováděn vždy do nové revize, pokud je verzování vypnuté, je prováděn do hlavní (jediné) revize.
...
Zároveň mohou obě aplikace aktuálně provádět změny pouze na stejném fyzickém zařízení, na kterém jsou nainstalovány - za předpokladu že serverová a klientská část aplikace jsou nainstalovány na různých zařízeních, musí být aplikace \textit{Updater} a \textit{Configurator} instalovány dvakrát.
