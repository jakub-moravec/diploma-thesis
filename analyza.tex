...
\begin{itemize}
	...
	\item{\textit{Kombinace operací pro čtení a zápis při vytváření nové revize}}: Modul \textit{Merger} při vytváření nové revize modelu datových toků v grafové databází provádí několik komplexních postproccessingových algoritmů, které vyžadují přístup pro čtení i zápis do pracovní otevřené revize (a jsou v některých případech prováděny paralelně). %TODO footnote on paralel
	\item{\textit{"Stop the world" operace}}: Je také definováno několik takzvaně "stop the world" operací, tedy operací, při kterých je znepřístupněna velká část, nebo přímo celá grafová databáze. Mezi tyto operace patří odstraňování starých revizí modelu datových toků\footnote{Odstranění starých revizí modelu datových toků z grafové databáze nelze implementovat pouhým odstraňováním objektů z databáze, je z velké části realizováno úpravou intervalu platnosti hran a musí tak dojít k průchodu celého modelu datových toků.}, nebo import či export kompletního \textit{dumpu} grafové databáze.
\end{itemize}
...
Transakce jsou řízeny programaticky (\textit{Programatic Transaction Model} \cite{Little04}) za pomoci implementace \textit{Spring TransactionTemplate}\footnote{\url{https://docs.spring.io/spring/docs/current/javadoc-api/org/springframework/transaction/support/TransactionTemplate.html}} kombinující \textit{Titan} transakce (převolávané pomocí \textit{Blueprints API}) a Java \textit{reentrant} zámky. \textit{Titan} v případě transakcí pouze přeposkytuje funkcionalitu používané podkladové databáze, kterou je v tuto chvíli \textit{embedded NoSQL databáze typu klíč-hodnota Persistit}\footnote{\url{https://github.com/pbeaman/persistit}}. Ta, stejně jako mnoho dalších grafových databází, implementuje \textit{Multi-Version Concurrency Control (MVCC)} (TODO link) využívající \textit{Snapshot Isolation} (TODO link) úroveň izolace transakcí (TODO zdroj). Jedná se o \textit{optimistický} transakční model, žádné databázové objekty (ani záznamy) nejsou zamykány, takže transakce probíhají v plné rychlosti bez čekání, může tak ale nastat situace, že ne všechny transakce je možné \textit{commitnout} a jedna (nebo více) z těchto transakcí tak musí být zrušena (a zopakována).
Synchronizace pomocí \textit{reentrant} zámků je používána pro přidělování transakcí, vlastní zámek je pro obecné transakce a vlastní pro \textit{read-only} transakce.

Další používanou synchronizační mechanikou je zamykání grafové databáze (respektive uzlů reprezentující jednotlivé revize) \textit{reentrant read-write} zámkem při několika operacích, které jsou součástí \textit{Mergeru} a importu a exportu \textit{dumpu} grafové databáze.

Poslední synchronizace na straně serveru je zamykání metadatové databáze nesoucí informace a zanalyzovaných skriptech pro účely kontroly licencí.




% FIXME otázka - je možné použít Spring deklarativní transakce spolus s Gremlinem?

Transakce v Perzistitu - optimistické, snapshot transakce, bez lockování
https://github.com/pbeaman/persistit/blob/master/src/main/java/com/persistit/Transaction.java

Transakce v Orientu - the same
https://orientdb.com/docs/2.1.x/Transactions.html

JanusGraph zdá se umožňuje pesimistické transakce, ale není to úplně dobře dokumentované
https://github.com/JanusGraph/janusgraph/blob/0b1b8237c5d4e8a41bde5f9f3b4cc6c66ab698bc/janusgraph-core/src/main/java/org/janusgraph/diskstorage/BackendTransaction.java
Minimálně BerkeleyDB umí taky locking
https://docs.oracle.com/cd/E17277_02/html/TransactionGettingStarted/txnconcurrency.html
